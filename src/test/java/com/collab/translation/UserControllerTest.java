...
class UserControllerTest {
    ...
    void shouldReturnBadRequestWithValidations() throws Exception {
        ...
        when(service.save(any(NewUser.class))).thenReturn(left(Validation.of(name, someValidation)));

        mockMvc.perform(post("/users")
                .content(mapper.writeValueAsBytes(newUser))
                .contentType(APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.value", is(name)))
                .andExpect(jsonPath("$.validations", contains(someValidation)));
    }
}
